...
- Homepage — centered directory with blog, gallery, tools links (needs redesign to match finalized design system)
- First-pass blog listing (/blog) and individual post pages (/blog/[slug]) — 3 placeholder posts, static data (no Supabase, no password protection yet)
- First-pass gallery page — currently at /art (WRONG — must be moved to /gallery). Uses custom lightbox (WRONG — must use yet-another-react-lightbox). Static data, no Supabase.
- First-pass diplomat plates tracker — currently at /diplomat (WRONG — must be moved to /tools/diplomat-plates). No sightings log, no country checklist, no Supabase.
- NavBar and PageWrapper shared components
- Design system finalized — see design system section below

⚠ technical debt from first pass (must fix):
- /art → /gallery (route rename + nav update)
- /diplomat → /tools/diplomat-plates (route rename, needs /tools directory page too)
- Replace custom lightbox with yet-another-react-lightbox
- Replace all static .ts data files with Supabase once database is set up
- Blog posts need password protection
- No admin system, no auth, no Tiptap editor yet
...
Status: first pass built at wrong route (/diplomat) — needs to be moved to /tools/diplomat-plates and features expanded.
...
**before building any new subpages, the admin system must be set up. do not skip this.**

The site needs a built-in admin system from the start. This should be done before expanding any content sections.
...
```
Before we build any more subpages, I need an admin system
...
—  Homepage — centered directory with blog, gallery, tools links (needs redesign to match finalized design system)
—  Blog listing (/blog) — first pass, static data, no password protection yet
—  Blog individual post pages (/blog/[slug]) — first pass, 3 placeholder posts, static data
—  Gallery page — first pass, WRONG ROUTE (/art, not /gallery), WRONG lightbox (custom, not yet-another-react-lightbox), static data
—  Diplomat plates tracker — first pass, WRONG ROUTE (/diplomat, not /tools/diplomat-plates), no sightings log, no country checklist, no Supabase
—  NavBar and PageWrapper shared components

**MUST FIX (technical debt from first pass)**
—  Rename /art → /gallery (route, nav link, all references)
—  Rename /diplomat → /tools/diplomat-plates (route, nav link)
—  Add /tools directory page
—  Replace custom gallery lightbox with yet-another-react-lightbox
—  Redesign homepage to match finalized design system (dark charcoal, no header, centered name + links, admin toggle square)

**IN PROGRESS**
—  (nothing currently in progress)

**PLANNED**
—  Admin system (NextAuth.js, /login page, admin mode toggle, 90-day session) — build this first
—  Supabase database setup (posts, gallery, countries, plate_sightings tables)
—  Blog: Tiptap editor in admin, publish/draft toggle, password protection on individual posts
—  Gallery: upload interface in admin, migrate to Supabase storage, yet-another-react-lightbox
—  Diplomat plates: expand to /tools/diplomat-plates, add sightings log, country checklist, admin logging form
—  Polish pass — responsive testing, typography, loading states, error handling, 404 page
